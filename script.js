...
function renderizarRanking(lista) {
    ...
    if (!ranking.length) {
        container.innerHTML = "<p style='color:#777'>Nenhum membro registado.</p>";
        return;
    }
...
window.adicionarPonto = async (id) => {
    ...
    if (!isAdminAtual) {
        alert('Apenas o Admin pode adicionar pontos.');
        return;
    }
    ...
    try {
        ...
        const atual = Math.max(0, Number(snap.data().pontos) || 0);
...
window.removerPontos = async (id) => {
    ...
    if (!isAdminAtual) {
        alert('Apenas o Admin pode remover pontos.');
        return;
    }
    ...
    try {
        ...
        const atual = Math.max(0, Number(snap.data().pontos) || 0);
        const novoTotal = Math.max(0, atual - 1);
...
window.closeModal = () => {
    ...
    const custom = document.getElementById('tourTierCustom');

    if (custom) {
        custom.classList.add('escondido');
        custom.value = '';
    }
};
...
window.verificarOutraTier = (valor) => {
    ...
    const inputCustom = document.getElementById('tourTierCustom');
...
window.fazerLogin = async () => {
    ...
    try {
        ...
        alert("Erro no login: E-mail ou palavra-passe incorretos.");
    }
};
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        if (statusText) {
            statusText.innerText =
                `LOGADO: ${user.email.split('@')[0].toUpperCase()}`;
        }

        if (statusDot) {
            statusDot.style.background =
                isAdminAtual ? "#ff3333" : "#61a0ff";
        }
    ...
    /* Atualiza os torneios depois de saber se é Admin */
    ...
    container.innerHTML = dados.map(d => `
        ...
        <article class='tour'>
            ...
            <span style='color:var(--red2); font-size:10px; font-weight:700; text-transform:uppercase;'>
                ${escapeHtml(d.tier || 'Geral')}
            </span>

            <h3>
                ${escapeHtml(d.nome || 'Torneio')}
            </h3>

            <p style='color:#aaa; font-size:11px; margin:8px 0;'>
                📅 ${escapeHtml(d.dia || '')}
                &nbsp;|&nbsp;
                ⏰ ${escapeHtml(d.horario || '')}
            </p>
...
window.adicionarTorneio = async () => {
    ...
    const nome =
        document.getElementById('tourNome')
        .value.trim();

    const dia =
        document.getElementById('tourDia')
        .value.trim();

    const horario =
        document.getElementById('tourHorario')
        .value.trim();

    const selectTier =
        document.getElementById('tourTier')
        .value;
...
function renderizarHAsMembros(dados) {
    ...
    container.innerHTML = dados.map(d => `
        ...
        <div class="ha-membro-box">
            ...
            <b>${escapeHtml(d.pokemon_name || '')}</b>

            <span class="forma">
                ${escapeHtml(d.form || 'Normal')}
            </span>

            <span class="ha-nome">
                HA: ${escapeHtml(d.ha_name || '')}
            </span>
...
window.adicionarHA = async () => {
    ...
    if (!isAdminAtual) {
        ...
        alert(
            "Apenas o Admin pode adicionar HAs."
        );
    ...
    const pokemon_name =
        document
            .getElementById('novoPoke')
            .value.trim();

    const form =
        document
            .getElementById('novoForm')
            .value.trim();

    const ha_name =
        document
            .getElementById('novoHa')
            .value.trim();

    const username =
        document
            .getElementById('novoDono')
            .value.trim();
